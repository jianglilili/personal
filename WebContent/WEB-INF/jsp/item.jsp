		...
		<div class="panel panel-default">
			<div class="panel-body">
				<form class="form-inline" method="get" 
				      action="${pageContext.request.contextPath }/item/list.action">
						...
						<select	class="form-control" id="itemName"  name="itemName">
							...
							<c:forEach items="${nameType}" var="item">
								<option value="${item.dict_id}"
								        <c:if test="${item.dict_id == itemName}"> selected</c:if>>
								    ${item.dict_item_name }
								</option>
							</c:forEach>
						</select>
			...
			<div class="col-lg-12">
				<div class="panel panel-default">
						...
						<tbody>
							<c:forEach items="${page.rows}" var="row">
								<tr>
									<td>${row.item_id}</td>
									<td>${row.item_name}</td>
									<td>${row.item_principal}</td>
									<td>
										<a href="#" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#itemEditDialog" onclick= "editItem(${row.item_id})">修改</a>
										<a href="#" class="btn btn-danger btn-xs" onclick="deleteItem(${row.item_id})">删除</a>
									</td>
								</tr>
							</c:forEach>
						</tbody>
					...
					<div class="col-md-12 text-right">
						<itheima:page url="${pageContext.request.contextPath }/item/list.action" />
					</div>
					<!-- /.panel-body -->
				</div>
				<!-- /.panel -->
